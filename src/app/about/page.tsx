...
export default function About() {
  return (
    <main className="pt-24 overflow-hidden relative">
      {/* Identity Hero: Fixed Top Spacing & Blur Reveal */}
      <section className="py-40 px-6 lg:px-0 relative overflow-hidden">
        <div className="section-container grid lg:grid-cols-[1.3fr_0.7fr] gap-32 items-end">
          ...
          <ScrollReveal direction="right" distance={80} blur={40} delay={0.4} className="relative aspect-square rounded-[4rem] overflow-hidden glass-frosted p-2 gentle-float mb-12">
            <Image
              src="/assets/devops.png"
              alt="Operational Logic"
              fill
              className="object-cover opacity-70 hover:opacity-100 transition-all duration-[4s]"
            />
          </ScrollReveal>
        </div>
      </section>

      {/* Practitioner vs Architect: Balanced High-Fi Bio */}
      ...
      {/* Vertical Roadmap: High-Density & Staggered */}
      <section className="py-40 lg:py-64 relative border-y border-white/5">
        <div className="section-container">
          <ScrollReveal direction="up" blur={20} className="mb-40">
          ...
          <div className="space-y-4px overflow-hidden rounded-[4rem] border border-white/10 bg-white/[0.01]">
            {roadmap.map((item, i) => (
              <ScrollReveal
                key={item.title}
                direction="none"
                delay={i * 0.2}
                blur={20}
                className="grid lg:grid-cols-[1.5fr_2fr] gap-12 group p-16 bg-[#0f172a] border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-all duration-1000"
              >
                <div className="space-y-6">
                  ...
                  <h3 className="text-4xl font-medium tracking-tight h-[1.2em]">{item.title}</h3>
                ...
                <div className="space-y-12">
                  ...
                  <ul className="grid md:grid-cols-2 gap-x-12 gap-y-10">
                    {item.points.map((point) => (
                      ...
                      >
                        <ChevronRight size={20} className="opacity-20 mt-1 flex-shrink-0" />
                        {point}
                      </li>
                    ))}
                  </ul>
